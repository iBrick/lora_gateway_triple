 ...
 *  Feb 28th, 2018
 *		- there is no longer is_binary flag, replaced by is_downlink
 *flag
 *		- the flags are then from left to right:
 *ack_requested|encrypted|with_appkey|is_downlink Feb 25th, 2018
 *      - use shared payload buffer for packet_sent and packet_received
 *      - use dedicated smaller buffer for ACK
 *  Feb 13th, 2018
 *      - fix bug in availableData() to set back the LoRa module into standby
 *mode. This affected only some radio modules November 10th, 2017
 ...
 *  Apr, 21th, 2017
 *      - change the way timeout are detected: exitTime=millis()+(unsigned
 *long)wait; then millis() < exitTime; Mar, 26th, 2017
 *      - insert delay(100) before setting radio module to sleep mode. Remove
 *unstability issue
 *      - (proposed by escyes -
 *https://github.com/CongducPham/LowCostLoRaGw/issues/53#issuecomment-289237532)
 *  Jan, 11th, 2017
 *      - fix bug in getRSSIpacket() when SNR < 0 thanks to John Rohde from
 *Aarhus University Dec, 17th, 2016
 ...
 *  Jan, 23rd, 2016
 *      - the packet format at transmission does not use the original Libelium
 *format anymore
 *      * the retry field is removed therefore all operations using retry will
 *probably not work well, not tested though
 *          - therefore DO NOT use sendPacketTimeoutACKRetries()
 *          - the reason is that we do not want to have a reserved byte after
 *the payload
 *      * the length field is removed because it is much better to get the
 *packet length at the reception side
 *      * after the dst field, we inserted a packet type field to better
 *identify the packet type: DATA, ACK, encryption, app key,...
 ...
 *ack_requested|encrypted|with_appkey|is_binary
 *          - ptype can be set with setPacketType(), see constant defined in
 *SX1272.h
 *          - the header length is then 4 instead of 5
 *  Jan, 16th, 2016
 *      - add support for SX1276, automatic detect
 *      - add LF/HF calibaration copied from LoRaMAC-Node. Don't know if it is
 ...
 *          - use following in your code if (loraMode==11) { e =
 *sx1272.setChannel(CH_18_868); } Nov, 13th, 2015
 *      - add CarrierSense() to perform some Listen Before Talk procedure
 *      - add dynamic ACK suport
 *          - compile with W_REQUESTED_ACK, retry field is used to indicate at
 *the receiver that an ACK should be sent
 *          - receiveWithTimeout() has been modified to send an ACK if retry is
 *1
 *          - at sender side, sendPacketTimeoutACK() has been modified to
 *indicate whether retry should be set to 1 or not in setPacket()
 *          - receiver should always use receiveWithTimeout() while sender
 *decides to use sendPacketTimeout() or sendPacketTimeoutACK() Jun, 2015
  ...
  if (0)
    if (_board == SX1276Chip) {
      ...
      writeRegister(REG_IMAGE_CAL,
                    (readRegister(REG_IMAGE_CAL) & RF_IMAGECAL_IMAGECAL_MASK) |
                        RF_IMAGECAL_IMAGECAL_START);
      while ((readRegister(REG_IMAGE_CAL) & RF_IMAGECAL_IMAGECAL_RUNNING) ==
...
uint8_t SX1272::ON(char *devpath) {
  ...
  // CAUTION
  // doing initialization as proposed by Libelium seems not to work for the
  // SX1276 so we decided to leave the default value of the SX127x, then
  // configure the radio when setting to LoRa mode
...
void SX1272::clearFlags() {
  ...
  if (_modem == LORA) { // LoRa mode
    ...
    writeRegister(REG_OP_MODE,
                  FSK_STANDBY_MODE);     // Stdby mode to write in registers
...
uint8_t SX1272::setFSK() {
  ...
  writeRegister(REG_PACKET_CONFIG1,
...
/*
 ...
 modulation. Returns: Integer that determines if there has been any error state
 = 2  --> The command has not been executed state = 1  --> There has been an
 error while executing the command state = 0  --> The command has been executed
 with no errors
*/
uint8_t SX1272::getMode() {
  ...
  if (_board == SX1272Chip) {
    _bandwidth =
        (value >> 6); // Storing 2 MSB from REG_MODEM_CONFIG1 (=_bandwidth)
    // added by C. Pham
  ...
  if (_board == SX1272Chip)
    _codingRate =
        (value >> 3) & 0x07; // Storing third, forth and fifth bits from
  else
    _codingRate = (value >> 1) &
                  0x07; // Storing 3-1 bits REG_MODEM_CONFIG1 (=_codingRate)
...
/*
 ...
 = 2  --> The command has not been executed state = 1  --> There has been an
 error while executing the command state = 0  --> The command has been executed
 with no errors state = -1 --> Forbidden command for this protocol Parameters:
...
int8_t SX1272::setMode(uint8_t mode) {
  ...
  if (state == -1) // if state = -1, don't change its value
  {
    ...
    switch (mode) { //      Different way to check for each mode:
      // (config1 >> 3) ---> take out bits 7-3 from REG_MODEM_CONFIG1
      // (=_bandwidth & _codingRate together) (config2 >> 4) ---> take out bits
      // 7-4 from REG_MODEM_CONFIG2 (=_spreadingFactor)
      ...
      case 1:
        ...
        if (_board == SX1272Chip) {
          ...
          // (config1 >> 1) ---> take out bits 7-1 from REG_MODEM_CONFIG1
          // (=_bandwidth & _codingRate together)
          if ((config1 >> 1) == 0x39)
            state = 0;
        }
...
/*
 ...
 mode. Returns: Integer that determines if there has been any error state = 2
 --> The command has not been executed state = 1  --> There has been an error
 while executing the command state = 0  --> The command has been executed with
 no errors
*/
uint8_t SX1272::getHeader() {
  ...
  if (bitRead(REG_MODEM_CONFIG1, theHeaderBit) ==
      0) { // explicit header mode (ON)
    _header = HEADER_ON;
    state   = 1;
...
#if (SX1272_debug_mode > 1)
      printf(
          "## Mandatory implicit header mode with spreading factor = 6 ##\n");
#endif
    } else {
      // added by C. Pham
      if (_board == SX1272Chip)
...
int8_t SX1272::setHeaderOFF() {
  ...
  if (_modem == FSK) { // header is not available in FSK mode
    ...
    if (_board == SX1272Chip)
      config1 =
          config1 | 0B00000100; // sets bit 2 from REG_MODEM_CONFIG1 = headerOFF
    else
      config1 =
          config1 | 0B00000001; // sets bit 0 from REG_MODEM_CONFIG1 = headerOFF
    ...
    if (bitRead(config1, theHeaderBit) ==
        HEADER_OFF) { // checking headerOFF taking out bit 2 from
                      // REG_MODEM_CONFIG1
      state   = 0;
      _header = HEADER_OFF;
...
uint8_t SX1272::getCRC() {
  ...
  if (_modem == LORA) { // LoRa mode
    ...
    if (bitRead(value, theCrcBit) == CRC_OFF) { // CRCoff
      _CRC = CRC_OFF;
#if (SX1272_debug_mode > 1)
...
uint8_t SX1272::setCRC_ON() {
  ...
  if (_modem == LORA) { // LORA mode
    ...
    config1 =
        readRegister(theRegister); // Save config1 to modify only the CRC bit

    if (_board == SX1272Chip)
      config1 =
          config1 | 0B00000010; // sets bit 1 from REG_MODEM_CONFIG1 = CRC_ON
    else
      config1 =
          config1 | 0B00000100; // sets bit 2 from REG_MODEM_CONFIG2 = CRC_ON
    ...
    if (bitRead(config1, theCrcBit) ==
        CRC_ON) { // take out bit 1 from REG_MODEM_CONFIG1 indicates
                  // RxPayloadCrcOn
      state = 0;
      _CRC  = CRC_ON;
#if (SX1272_debug_mode > 1)
    ...
    config1 =
        config1 | 0B00010000; // set bit 4 and 3 from REG_MODEM_CONFIG1 = CRC_ON
    writeRegister(REG_PACKET_CONFIG1, config1);

    state = 1;

    config1 = readRegister(REG_PACKET_CONFIG1);
    if (bitRead(config1, 4) ==
...
uint8_t SX1272::setCRC_OFF() {
  ...
  if (_modem == LORA) { // LORA mode
    ...
    config1 = readRegister(theRegister);
    if ((bitRead(config1, theCrcBit)) ==
        CRC_OFF) { // take out bit 1 from REG_MODEM_CONFIG1 indicates
                   // RxPayloadCrcOn
      state = 0;
      _CRC  = CRC_OFF;
#if (SX1272_debug_mode > 1)
    ...
    config1 = readRegister(REG_PACKET_CONFIG1);
    if (bitRead(config1, 4) ==
        CRC_OFF) { // take out bit 4 from REG_PACKET_CONFIG1 indicates
                   // RxPayloadCrcOn
      state = 0;
      _CRC  = CRC_OFF;
#if (SX1272_debug_mode > 1)
...
uint8_t SX1272::setSF(uint8_t spr) {
  ...
  if (_modem == FSK) {
    ...
    writeRegister(REG_OP_MODE, LORA_STANDBY_MODE); // LoRa standby mode
    config1 = (readRegister(REG_MODEM_CONFIG1)); // Save config1 to modify only
                                                 // the LowDataRateOptimize
    config2 = (readRegister(
        REG_MODEM_CONFIG2)); // Save config2 to modify SF value (bits 7-4)
    switch (spr) {
      case SF_6:
        config2 =
            config2 & 0B01101111; // clears bits 7 & 4 from REG_MODEM_CONFIG2
        config2 =
            config2 | 0B01100000; // sets bits 6 & 5 from REG_MODEM_CONFIG2
      ...
      case SF_8:
        config2 =
            config2 & 0B10001111; // clears bits 6, 5 & 4 from REG_MODEM_CONFIG2
        config2 = config2 | 0B10000000; // sets bit 7 from REG_MODEM_CONFIG2
        break;
      case SF_9:
        config2 =
            config2 & 0B10011111; // clears bits 6, 5 & 4 from REG_MODEM_CONFIG2
        config2 =
            config2 | 0B10010000; // sets bits 7 & 4 from REG_MODEM_CONFIG2
        break;
      case SF_10:
        config2 =
            config2 & 0B10101111; // clears bits 6 & 4 from REG_MODEM_CONFIG2
        config2 =
            config2 | 0B10100000; // sets bits 7 & 5 from REG_MODEM_CONFIG2
        break;
      case SF_11:
        config2 = config2 & 0B10111111; // clears bit 6 from REG_MODEM_CONFIG2
        config2 =
            config2 | 0B10110000; // sets bits 7, 5 & 4 from REG_MODEM_CONFIG2
        getBW();

        // modified by C. Pham
        if (_bandwidth ==
            BW_125) { // LowDataRateOptimize (Mandatory with SF_11 if BW_125)
          if (_board == SX1272Chip)
      ...
      case SF_12:
        config2 =
            config2 & 0B11001111; // clears bits 5 & 4 from REG_MODEM_CONFIG2
        config2 =
            config2 | 0B11000000; // sets bits 7 & 6 from REG_MODEM_CONFIG2
        if (_bandwidth ==
            BW_125) { // LowDataRateOptimize (Mandatory with SF_12 if BW_125)
          // modified by C. Pham
          if (_board == SX1272Chip)
            config1 = config1 | 0B00000001;
          else {
    ...
    if (_board == SX1272Chip) {
      config1 =
          (readRegister(REG_MODEM_CONFIG1)); // Save config1 to check update
      config2 =
          (readRegister(REG_MODEM_CONFIG2)); // Save config2 to check update
      // comment by C. Pham
      // (config2 >> 4) ---> take out bits 7-4 from REG_MODEM_CONFIG2
      // (=_spreadingFactor) bitRead(config1, 0) ---> take out bits 1 from
      // config1 (=LowDataRateOptimize) config2 is only for the AgcAutoOn
      configAgc = config2;
      theLDRBit = 0;
    } else {
      config1 =
          (readRegister(REG_MODEM_CONFIG3)); // Save config1 to check update
      config2 = (readRegister(REG_MODEM_CONFIG2));
    ...
    switch (spr) {
      case SF_6:
        if (((config2 >> 4) == spr) && (bitRead(configAgc, 2) == 1) &&
            (_header == HEADER_OFF)) {
          state = 0;
        }
        break;
      case SF_7:
        ...
        break;
      case SF_8:
        if (((config2 >> 4) == 0x08) && (bitRead(configAgc, 2) == 1)) {
          state = 0;
        }
        break;
      case SF_9:
        if (((config2 >> 4) == 0x09) && (bitRead(configAgc, 2) == 1)) {
          state = 0;
        }
        break;
      case SF_10:
        if (((config2 >> 4) == 0x0A) && (bitRead(configAgc, 2) == 1)) {
          state = 0;
        }
        break;
      case SF_11:
        if (((config2 >> 4) == 0x0B) && (bitRead(configAgc, 2) == 1) &&
            (bitRead(config1, theLDRBit) == 1)) {
          state = 0;
        }
        break;
      case SF_12:
        if (((config2 >> 4) == 0x0C) && (bitRead(configAgc, 2) == 1) &&
            (bitRead(config1, theLDRBit) == 1)) {
          state = 0;
        }
        break;
...
#if (SX1272_debug_mode > 1)
      printf(
          "** There has been an error while setting the spreading factor **");
      printf("\n");
#endif
    }
  }
...
int8_t SX1272::setBW(uint16_t band) {
  ...
  writeRegister(REG_OP_MODE, LORA_STANDBY_MODE); // LoRa standby mode
  config1 =
      (readRegister(REG_MODEM_CONFIG1)); // Save config1 to modify only the BW
  ...
  if (_board == SX1272Chip) {
    switch (band) {
      case BW_125:
        config1 =
            config1 & 0B00111111; // clears bits 7 & 6 from REG_MODEM_CONFIG1
        getSF();
        if (_spreadingFactor ==
    ...
    switch (band) {
      case BW_125:
        ...
        if (_spreadingFactor == 11 ||
            _spreadingFactor == 12) { // LowDataRateOptimize (Mandatory with
                                      // BW_125 if SF_11 or SF_12)
          byte config3 = readRegister(REG_MODEM_CONFIG3);
          config3      = config3 | 0B00001000;
          writeRegister(REG_MODEM_CONFIG3, config3);
        }
  ...
  // added by C. Pham
  if (_board == SX1272Chip) {
    // (config1 >> 6) ---> take out bits 7-6 from REG_MODEM_CONFIG1
    // (=_bandwidth)
    switch (band) {
      case BW_125:
        if ((config1 >> 6) == SX1272_BW_125) {
          state = 0;
          if (_spreadingFactor == 11) {
            if (bitRead(config1, 0) == 1) { // LowDataRateOptimize
              state = 0;
            } else {
              state = 1;
            }
...
int8_t SX1272::getCR() {
  ...
  if (_modem == FSK) {
    ...
    if (_board == SX1272Chip) {
      ...
      // take out bits 7-1 from REG_MODEM_CONFIG1 indicates _bandwidth &
      // _codingRate
...
int8_t SX1272::setCR(uint8_t cod) {
  ...
  config1 =
      readRegister(REG_MODEM_CONFIG1); // Save config1 to modify only the CR
  ...
  if (_board == SX1272Chip) {
    switch (cod) {
      case CR_5:
        config1 =
            config1 & 0B11001111; // clears bits 5 & 4 from REG_MODEM_CONFIG1
        config1 = config1 | 0B00001000; // sets bit 3 from REG_MODEM_CONFIG1
        break;
      case CR_6:
        config1 =
            config1 & 0B11010111; // clears bits 5 & 3 from REG_MODEM_CONFIG1
        config1 = config1 | 0B00010000; // sets bit 4 from REG_MODEM_CONFIG1
        break;
      case CR_7:
        config1 = config1 & 0B11011111; // clears bit 5 from REG_MODEM_CONFIG1
        config1 =
            config1 | 0B00011000; // sets bits 4 & 3 from REG_MODEM_CONFIG1
        break;
      case CR_8:
        config1 =
            config1 & 0B11100111; // clears bits 4 & 3 from REG_MODEM_CONFIG1
  ...
  // ((config1 >> 3) & 0B0000111) ---> take out bits 5-3 from REG_MODEM_CONFIG1
  // (=_codingRate)
  switch (cod) {
    case CR_5:
...
int8_t SX1272::setPower(char p) {
  ...
  if (_board == SX1272Chip) {
    ...
    // so x= 14dBm (PA);
    // when p=='X' for 20dBm, value is 0x0F and RegPaDacReg=0x87 so 20dBm is
    // enabled
...
int8_t SX1272::setPacketLength() {
  ...
  // added by C. Pham
  // if gateway is in rawFormat mode for packet reception, it will also send in
  // rawFormat unless we switch it back to normal format just for transmission,
  // e.g. for downlink transmission
  if (_rawFormat)
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = -1 --> Forbidden command for this protocol
*/
int8_t SX1272::getSNR() { // getSNR exists only in LoRa mode
  int8_t state = 2;
  byte value;
...
uint8_t SX1272::getRSSI() {
  ...
  if (_modem == LORA) {
    ...
    for (int i = 0; i < total; i++) {
      ...
      _RSSI = -(OFFSET_RSSI + (_board == SX1276Chip ? 18 : 0)) +
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = -1 --> Forbidden command for this protocol
*/
int16_t SX1272::getRSSIpacket() { // RSSIpacket only exists in LoRa
  int8_t state = 2;
  ...
  if (_modem == LORA) { // LoRa mode
    ...
    if (state == 0) {
      ...
      if (_SNR < 0) {
        ...
        // then for SX1276 when using low-frequency (i.e. 433MHz) then we use
        // -164
        //_RSSIpacket =
        //-(OFFSET_RSSI+(_board==SX1276Chip?18:0)+(_channel<CH_04_868?7:0)) +
        //(double)_RSSIpacket + (double)_rawSNR*0.25;
...
/*
 ...
 battery chemistries. Returns: Integer that determines if there has been any
 error state = 2  --> The command has not been executed state = 1  --> There has
 been an error while executing the command state = 0  --> The command has been
 executed with no errors Parameters: rate: value to compute the maximum current
 supply. Maximum current is 45+5*'rate' [mA]
*/
...
/*
 ...
 chemistries. Returns: Integer that determines if there has been any error state
 = 2  --> The command has not been executed state = 1  --> There has been an
 error while executing the command state = 0  --> The command has been executed
 with no errors state = -1 --> Forbidden parameter value for this function
 Parameters:
...
uint8_t SX1272::receive() {
  ...
  // writeRegister(REG_LNA, 0x23);			// Important in
  // reception
  // modified by C. Pham
  ...
  writeRegister(REG_FIFO_RX_BYTE_ADDR,
                0x00); // Setting current value of reception buffer pointer

  // clearFlags();						// Initializing
  // flags

  // state = 1;
  if (_modem == LORA) {                  // LoRa mode
    state = setPacketLength(MAX_LENGTH); // With MAX_LENGTH gets all packets
...
// added by C. Pham
// receiver always use receivePacketTimeout() but check the retry field to see
// whether an ACK is requested note that sender should either use
...
#  endif
          ...
          state_f =
              1; // There has been an error with the 'sendWithTimeout' function
        }
      } else {
    ...
    // we need to conserve state_f=3 to indicate that no packet has been
...
/*
 ...
 incorrect state = 3  --> The command has been executed but there is no packet
 received state = 2  --> The command has not been executed state = 1  --> There
 has been an error while executing the command state = 0  --> The command has
 been executed with no errors
*/
uint8_t SX1272::receivePacketTimeoutACK(uint16_t wait) {
  ...
  if( (state == 0) || (state == 3) )
  {
      ...
      if( state == 0 )
      {
          ...
          {
              state_f = 1; // There has been an error with the 'sendWithTimeout'
function
          }
      }
      else
...
/*
 ...
 MAX_TIMEOUT expires. Returns: Integer that determines if there has been any
 error state = 2  --> The command has not been executed state = 1  --> There has
 been an error while executing the command state = 0  --> The command has been
...
boolean SX1272::availableData(uint16_t wait) {
...
#ifdef W_NET_KEY
                         // if we actually wait for an ACK, there is no net key
                         // before ACK data
        if (_requestACK == 0) {
  ...
  // We use _hreceived because we need to ensure that _destination value is
  // correctly updated and is not the _destination value from the previously
  // packet
  if (_hreceived) { // Checking destination
#if (SX1272_debug_mode > 0)
    ...
    if (forme &&
        ((_destination == _nodeAddress) || (_destination == BROADCAST_0)))
#else
                    // modified by C. Pham
...
/*
 ...
 expires. Returns:  Integer that determines if there has been any error state =
 2  --> The command has not been executed state = 1  --> There has been an error
 while executing the command state = 0  --> The command has been executed with
 no errors
*/
...
/*
 ...
 time. Returns:  Integer that determines if there has been any error state = 3
 --> The command has been executed but packet has been incorrectly received
   state = 2  --> The command has not been executed
   state = 1  --> There has been an error while executing the command
   state = 0  --> The command has been executed with no errors
...
int8_t SX1272::getPacket(uint16_t wait) {
  ...
  if (_modem == LORA) { // LoRa mode
    ...
    // while( (bitRead(value, 6) == 0) && (millis() - previous < (unsigned
    // long)wait) )
    ...
    if ((bitRead(value, 6) == 1)) {
      ...
      if (bitRead(readRegister(REG_HOP_CHANNEL), 6)) {
        ...
        _reception = CORRECT_PACKET;
#if (SX1272_debug_mode > 0)
        printf("## Packet supposed to be correct as CrcOnPayload is off at "
               "transmitter ##\n");
#endif
      }
    }
  ...
  if (p_received) {
    ...
    if (_modem == LORA) {
      ...
      if (!_rawFormat)
        packet_received.dst =
            readRegister(REG_FIFO); // Storing first byte of the received packet
      else
      ...
      if ((bitRead(value, 2) == 0) && (bitRead(value, 1) == 0)) {
        ...
        packet_received.dst =
            _destination; // Storing first byte of the received packet
      }
    }
    ...
    if (!_rawFormat) {
      packet_received.type =
          readRegister(REG_FIFO); // Reading second byte of the received packet
      ...
      if (((packet_received.type & PKT_TYPE_MASK) != PKT_TYPE_DATA) &&
        ...
        packet_received.src = readRegister(
            REG_FIFO); // Reading second byte of the received packet
        packet_received.packnum =
            readRegister(REG_FIFO); // Reading third byte of the received packet
        // packet_received.length = readRegister(REG_FIFO);	// Reading
        // fourth byte of the received packet
      }
    } else {
...
uint8_t SX1272::setPacket(uint8_t dest, char *payload) {
  ...
  if (_retries == 0) { // Updating this values only if is not going to re-send
                       // the last packet
  ...
  writeRegister(REG_FIFO_ADDR_PTR,
                0x80); // Setting address pointer in FIFO data buffer
  if (state == 0) {
    state = 1;
...
uint8_t SX1272::setPacket(uint8_t dest, uint8_t *payload) {
  ...
  _reception =
      CORRECT_PACKET;  // Updating incorrect value to send a packet (old or new)
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t SX1272::sendPacketMAXTimeout(uint8_t dest,
                                     uint8_t *payload,
                                     uint16_t length16) {
...
*/
uint8_t
SX1272::sendPacketTimeout(uint8_t dest, uint8_t *payload, uint16_t length16) {
  uint8_t state   = 2;
  uint8_t state_f = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t SX1272::sendPacketTimeout(uint8_t dest,
                                  uint8_t *payload,
                                  uint16_t length16,
                                  uint16_t wait) {
  uint8_t state   = 2;
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t SX1272::sendPacketMAXTimeoutACK(uint8_t dest,
                                        uint8_t *payload,
                                        uint16_t length16) {
...
uint8_t SX1272::sendPacketTimeoutACK(uint8_t dest, char *payload) {
  ...
  state =
      sendPacketTimeout(dest, payload); // Sending packet to 'dest' destination
...
/*
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = 0  --> The command has been executed with no errors
*/
uint8_t SX1272::sendPacketTimeoutACK(uint8_t dest,
                                     uint8_t *payload,
                                     uint16_t length16) {
  uint8_t state   = 2;
  uint8_t state_f = 2;
...
*/
uint8_t
SX1272::sendPacketTimeoutACK(uint8_t dest, char *payload, uint16_t wait) {
  uint8_t state   = 2;
  uint8_t state_f = 2;
...
/*
 ...
 time. Returns: Integer that determines if there has been any error state = 2
 --> The ACK has not been received state = 1  --> The N-ACK has been received
 with no errors state = 0  --> The ACK has been received with no errors
 Parameters:
   wait: time to wait while there is no a valid header received.
*/
...
/*
 ...
 error. Returns: Integer that determines if there has been any error state = 2
 --> The command has not been executed state = 1  --> There has been an error
 while executing the command state = 0  --> The command has been executed with
 no errors
*/
...
/*
 ...
 no errors
*/
uint8_t SX1272::sendPacketMAXTimeoutACKRetries(uint8_t dest,
                                               uint8_t *payload,
                                               uint16_t length16) {
...
/*
 ...
 no errors
*/
uint8_t SX1272::sendPacketTimeoutACKRetries(uint8_t dest,
                                            uint8_t *payload,
                                            uint16_t length16) {
  uint8_t state = 2;
...
/*
 ...
 Returns: Integer that determines if the number of requested CAD have been
 successfull state = 2  --> The command has not been executed state = 1  -->
 There has been an error while executing the command state = 0  --> The command
    ...
    do {
      ...
      if (bitRead(value, 2) == 1) {
        ...
        counter--;
                ...
                // to many errors, so exit by indicating that channel is not
                // free
                if (!retryCAD)
                  failedCAD = true;
      }
...
uint16_t SX1272::getToA(uint8_t pl) {
  ...
  bw =
      (_bandwidth == BW_125) ? 125e3 : ((_bandwidth == BW_250) ? 250e3 : 500e3);
  ...
  // must add 4 to the programmed preamble length to get the effective preamble
  // length
...
// we advise using _send_cad_number=3 for a SIFS and _send_cad_number=9 for a
// DIFS prior to send any data
...
long SX1272::getRemainingToA() {
  ...
  // we compare to the end of cycle so that millis() rollover is taken into
  // account using unsigned long modulo operation
