...
void initSafeWrite()
{
	...
	if (spi_sem != SEM_FAILED)
	{ // We are the first program of the pack reached that point
...
{
	byte told= 0;
	if (Pin == SX1272_SS && State == SS_STATE_BLOCK && ss_state != SS_STATE_BLOCK)
	{
		//int sv= 0;
	...
	if (Pin == SX1272_SS && State == SS_STATE_FREE  && ss_state == SS_STATE_BLOCK )
	{
		sem_post(spi_sem);
		//printf("unlock\r\n");
	}
	if (Pin == SX1272_SS) ss_state= State;
}
