...
def test_network_available():
    if key_MQTT.MQTT_server == "localhost":
        return True
    ...
    while(not connection and iteration < 4):
        try:
            ...
            response = urllib2.urlopen(
                'http://'+key_MQTT.MQTT_server, timeout=3)
            connection = True
...
def send_data(data, src, nomenclatures, tdata):
    ...
    while i < len(data)-2:
        ...
        if nomenclatures == '':
            cmd = 'mosquitto_pub -h '+key_MQTT.MQTT_server+' -t ' + \
                data[0]+'/'+data[1]+'/'+src+' -m \"'+data[i+2]+'\"'
        else:
            cmd = 'mosquitto_pub -h '+key_MQTT.MQTT_server+' -t ' + \
                data[0]+'/'+data[1]+'/'+src+'/' + \
                nomenclatures[i]+' -m \"'+data[i+2]+'\"'
...
def main(ldata, pdata, rdata, tdata, gwid):
    print(ldata, pdata, rdata, tdata, gwid)
    ...
    if (src_str in key_MQTT.source_list) or (len(key_MQTT.source_list) == 0):
        ...
        if True:
            ...
            if ord(decoded[0]) == 255:
                ...
                data = subprocess.Popen(
                    ['python3', '/home/pi/lora_gateway/decipher.py', data, key_MQTT.appskey, key_MQTT.nwkskey]).stdout.read().strip()
                print('data now: ' + data)
        ...
        # or encapsulated encrypted
        elif ptype & 0x40 == 0x40 or ptype & 0x80 == 0x80 or ptype & 0x04 == 0x04:
            nomenclatures = ''
            data = ['', '']
            data.append(ldata)
            MQTT_uploadData(nomenclatures, data,
                            key_MQTT.sensor_name+src_str, tdata)
            ...
            if nsharp == 0:
                ...
                data_array = [data_array[0], data_array[1]] + \
                    re.split("/", data_array[2])

                # at the end data_array contains
                # ["project", "organisation_name", "s1", s1value, "s2", s2value, ...]

            # just in case we have an ending CR or 0
            data_array[len(data_array) -
                       1] = data_array[len(data_array)-1].replace('\n', '')
            data_array[len(data_array) -
                       1] = data_array[len(data_array)-1].replace('\0', '')
