...
void SPIPi::begin(char *devpath) {
  if (SPI_DEV1_init(SPIDEV_BYTES_NUM, SPIDEV1_BUS_SPEED_HZ, SPI_SS_LOW,
                    SPIDEV_DELAY_US, SPIDEV_DATA_BITS_NUM, SPI_MODE0) == -1,
      devpath)
...
void SPIPi::setClockDivider(uint16_t divider) {
  //    volatile uint32_t* paddr = (volatile uint32_t*)spi0 +
  //    BCM2835_SPI0_CLK/4; ch_peri_write(paddr, divider);
}

void SPIPi::setDataMode(uint8_t mode) {
  //    volatile uint32_t* paddr = (volatile uint32_t*)spi0 + BCM2835_SPI0_CS/4;
  // Mask in the CPO and CPHA bits of CS
  //    ch_peri_set_bits(paddr, mode << 2, BCM2835_SPI0_CS_CPOL |
...
void SPIPi::transfernb(char *tbuf, char *rbuf, uint32_t len) {
  SPIDEV1_transfer(reinterpret_cast<unsigned char *>(tbuf),
                   reinterpret_cast<unsigned char *>(rbuf), len);
}
...
void pinMode(int pin, Pinmode mode) {
  gpio_set_dir(beaglebonePinNumber(pin),
               (mode == OUTPUT) ? OUTPUT_PIN : INPUT_PIN);
}
...
long millis() {
  ...
  elapsedTime +=
      (end_point.tv_usec - start_program.tv_usec) / 1000.0; // us to ms
  return elapsedTime;
}
