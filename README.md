...
--------------------
На устройстве предусмотрены три светодиода индикации - `power` (`LED1`), `alarm` (`LED3`), `radio` (`LED4`).
По умолчанию при загрузке светодиод `power` активен, остальные - нет. Для управления светодиодами следует
записывать в файл `/sys/class/leds/power/brightness`, `/sys/class/leds/alarm/brightness`, или `/sys/class/leds/radio/brightness`
