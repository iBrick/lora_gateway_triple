...
Все инструкции написаны из предположения что вы находитесь в директории проекта, `/home/pi/lora_gateway`.
...
- для запуска одного коммуникационного модуля без постобработки команд:
(тут `n` это номер модуля - 0, 1, или 2) выполните скрипт `./scripts/Mn`

- для запуска полной подсистемы связи выполните скрипт `./scripts/bb-triple-restart`
Логи выполнения будут помодульно сложены в директорию `log`.

Авторизация посредством ключа One Wire
--------------------------------------
В директории `/sys/bus/1w` при подключении ключа к считывателю будут возникать директории с именами,
соответствующими ключу подключённого ключа. Внутри каждой из этих директорий также имеется файл `id`,
в котором ключ ключа содержится в бинарном виде, что удобнее для программной обработки, а список директорий
можно получать фильтрованием их списка. Через некоторое время после отключения ключа директории пропадают.
...
--------------------
Входящие сообщения от LoRa-устройств преобразуются из бинарной формы в base64-кодированное сообщение,
после чего транслируются по MQTT в канал, пронумерованный согласно LoRa-адресу устройства-отправителя.
...
- распаковку сообщения по протоколу Uplink,
...
- регистрацию и удаление из системы датчиков,

- изменение настроек датчиков,

- оповещение о сигналах тревоги, приходящих с датчиков,

- аутентификацию пользователей по ключу-"таблетке", а также регистрацию и удаление таких ключей,

- отключение датчиков или отвязку их от системы.

Некоторые из операций выполняются мгновенно, другие требуют дождаться подтверждения со стороно датчика.
Интерфейс программы позволяет различать эти ситуации и видеть прогресс выполнения задания.
...
Работа зарегистрированного датчика - LoRa-модуль
------------------------------------------------
Периодически формируется отчёт о текущем состоянии системы, если это указано в конфигурации, то отчёт шифруется,
и передаётся на отправку без требования ACK. Через определённый интервал времени открывается временное окно на
приём ответа от базы, содержащего изменения для конфигурации датчика.
...
------------------------------------------------
Сначала с периодом в 5 секунд выполняется проверка наличия троса, цикл не прерывается пока трос не вставлен.
